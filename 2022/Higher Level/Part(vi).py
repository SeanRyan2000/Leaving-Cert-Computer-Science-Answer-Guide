...
largest_value = 0
for i in range(6):
    if frequencies[i] > largest_value: # if the current value is larger than the current largest value
        largest_value = frequencies[i] # set the largest value to the current value
        largest_value_index = i # store the index of the largest value

# as arrays start at 0 we add 1 to the index to get the dice number
print(largest_value_index + 1, "was rolled the most, often (", largest_value, "times)")
